...
function generateManageOpeningTimesResponse(actionType, openingTime, closingTime) {
    console.log('Checking if ActionType is provided:', actionType);
    if (!actionType) {
        console.log('ActionType not provided. Eliciting slot ActionType.');
        return {
            sessionState: {
                dialogAction: {
                    type: 'ElicitSlot',
                    slotToElicit: 'ActionType', 
                },
                intent: {
            ...
            messages: [
                {
                    ...
                    content: 'Do you want to view or update your timings?',
                },
            ],
        };
    }
    ...
    return axios.post('https://wzdh0w0265.execute-api.us-east-1.amazonaws.com/prod/storeRestaurantDetails', payload)
        .then((response) => {
            ...
            if (response.data === true) {
                ...
                console.log('Opening times updated successfully:', responseMessage);
                return {
                    sessionState: {
                        dialogAction: {
                            type: 'Close',
                ...
                console.log('Failed to update opening hours. Response data:', response.data);
                return {
                    sessionState: {
                        dialogAction: {
                            type: 'Close',
        ...
        .catch((error) => {
            ...
            return {
                ...
                messages: [
                    {
                        contentType: 'PlainText',
                        content: 'Failed to update opening hours. Please try again later.',
                    },
                ],
            };
        });
...
module.exports = async (event) => {
    ...
    if (intentName === 'ManageOpeningTimes') {
        const actionTypeSlot = event.sessionState.intent.slots.ActionType;
        const openingTimeSlot = event.sessionState.intent.slots.OpeningTime;
        const closingTimeSlot = event.sessionState.intent.slots.ClosingTime;

        const actionType = actionTypeSlot && actionTypeSlot.value ? actionTypeSlot.value.originalValue : null;
        let openingTime = openingTimeSlot && openingTimeSlot.value ? openingTimeSlot.value.resolvedValues[0] : null;
        let closingTime = closingTimeSlot && closingTimeSlot.value ? closingTimeSlot.value.resolvedValues[0] : null;

        if (openingTime && !closingTime) {
            const [hours, minutes] = openingTime.split(':').map(Number);
            const closingHours = (hours + 6) % 24;
            closingTime = `${closingHours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;
        }
        
        console.log('Intent is ManageOpeningTimes. Extracted slots:', actionType, openingTime, closingTime);

        return generateManageOpeningTimesResponse(actionType, openingTime, closingTime);
    }

    console.log('Intent is not ManageOpeningTimes. Returning default response.');
    return {
        statusCode: 200,
